...
#let columns-content(..args) = {
  ...
  grid(columns: colwidths, gutter: thisgutter, ..bodies)
}

#let section-without-page(content) = heading(level:2, outlined:false)[#content]

#let interlude(text) = metadata((text: text))
...
#let project(
  ...
  set page(paper: "presentation-" + aspect-ratio, margin: 0pt)

  set list(tight: true, indent: 0.27cm, body-indent: 0.7cm, marker: (
    context{ place(center, dy: -0.25em, text(size: 1.5em, fill: main-color-state.at(here()), "▶")) },
    context{ place(center, dy: -0.2em, text(size: 1.3em, fill: main-color-state.at(here()), "■")) },
  ))
  ...
  let slide-polygon() = {
    place(
      top + left,
      context{ polygon(fill: main-color-state.at(here()), (0cm, 0cm), (0cm, 3cm), (0.4cm, 3cm), (0.4cm, 0cm)) },
    )
  }
  ...
  page(margin: 0pt)[
    ...
    #place(polygon(fill: main-color, (100%, 0%), (55%, 0%), (80%, 100%), (100%, 100%)))
  ...
  place(top + left, dy: -4cm, block(
    height: 3cm,
    width: if logo != none { 100% - 2.5cm } else { 100% },
    align(horizon, text(size: heading-2-size, weight: "regular", index-title)),
  ))
  context{
  ...
  show link: it => underline(context{ text(fill: main-color-state.at(here()))[#it] })

  let colored-page(caption) = [
    #section-page.update(x => true)
    #pagebreak(weak: true)
    #set text(text-size)
    #set text(heading-1-size, fill: white, weight: "bold")
    #move(dy: -1.5cm, place(horizon, pad(right: 5cm, caption)))
  ]

  show heading.where(level: 1): it => colored-page(upper(it.body))
  show metadata: it => colored-page(it.value.at("text"))

  show heading.where(level: 2): it => {
    if (it.outlined) {
      section-page.update(x => false)
      pagebreak()
      set text(text-size)
      ...
      place(
        top + left,
        dy: -4cm,
        block(height: 3cm, width: if logo != none { 100% - 2.5cm } else { 100% }, align(horizon, it.body)),
      )
    }
  }
    ...
    context{
      ...
      place(
        ...
        block(
          ...
          align(
            horizon,
            [#block(below: 0em, above: 0em, text(heading2, size: heading-2-size, weight: "regular")) #block(below: 0em, above: 0.65em, text(it.body, size: text-size, weight: "regular"))],
          ),
        ),
      )
    }
